...
def fake_transport() -> httpx.MockTransport:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if body.get("client") == "local":
            if body.get("fun") == "mine.get":
                CALLS.append(body)
                if DENY_MINE:
                    return httpx.Response(500, json={})
                fun = (body.get("arg") or ["*", ""])[1]
                data = {"web-01": ["10.0.0.1"]} if fun == "network.ip_addrs" \
                    else {}
                return httpx.Response(200, json={"return": [{
                    body.get("tgt"): data}]})
        return httpx.Response(200, json={"return": [{}]})
...
def query(c, **params):
    return c.get("/mine/", query_string={
        "tgt": "*", "tgt_type": "glob", "fun": "network.ip_addrs",
        **params})
...
def test_group_target_resolves_to_list():
    CALLS.clear()
    html = make_client().get("/mine/", query_string={
        "tgt": "g", "tgt_type": "group",
        "fun": "network.ip_addrs"}).data.decode()
...
def test_viewer_can_read():
    ...
    rv = viewer.get("/mine/", query_string={
        "tgt": "*", "tgt_type": "glob", "fun": "network.ip_addrs"})
...
def test_minion_tab_shows_stored_value():
    html = make_client().get(
        "/minions/web-01?tab=mine&mine_fun=network.ip_addrs").data.decode()
    assert "10.0.0.1" in html


def test_minion_tab_empty_explains():
    html = make_client().get(
        "/minions/web-01?tab=mine&mine_fun=nosuch").data.decode()
    assert "nothing stored" in html
    assert "mine_functions" in html
...
def test_minion_tab_denial_shows_error():
    ...
    try:
        html = make_client().get(
            "/minions/web-01?tab=mine&mine_fun=network.ip_addrs"
        ).data.decode()
    finally:
        DENY_MINE = False
