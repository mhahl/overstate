...
def seed_returns(jid, results):
    ...
    for mid, success in results:
        session.add(JobReturn(jid=jid, minion_id=mid, success=success,
                              retcode=0, payload={}))
    session.commit()
...
def test_split_roster_count_percent_remainder():
    ...
    assert split_roster(roster, "percent", 40) == [["a", "b"], ["c", "d"],
                                                  ["e"]]
...
def test_parse_batch_fields():
    ...
    assert parse_batch_fields({"batch_mode": "count", "batch_size": "x",
                               "stop_after": "1"}) is None
    assert parse_batch_fields({"batch_mode": "count", "batch_size": "2",
                               "stop_after": "0"}) is None
    assert parse_batch_fields({"batch_mode": "percent", "batch_size": "25",
                               "stop_after": "3"}) == {
...
def test_resolve_batch_roster(app):
    ...
    with app.app_context():
        ...
        assert resolve_batch_roster("web-01,db-01", "list") == ["db-01",
                                                                "web-01"]
...
def test_gate_trips_and_stops(monkeypatch, app):
    ...
    with app.app_context():
        session = get_session()
        session.add(Job(jid="batch-g1", fun="test.ping", tgt="web-*",
                        tgt_type="glob", user="admin", batch_group="g1",
                        batch_state={"status": "running"}))
        session.commit()
        seed_returns("w1", [("web-01", False)])
        out = run_wave_batch("g1", [["web-01"], ["web-02"]], "test.ping",
                             [], 1, "admin", wave_timeout=0)
    assert out == {"group": "g1", "status": "stopped", "failures": 1,
                   "waves": 2}
...
def test_complete_batch_and_audit(monkeypatch, app):
    ...
    with app.app_context():
        session = get_session()
        session.add(Job(jid="batch-g2", fun="test.ping", tgt="*", tgt_type="glob",
                        user="admin", batch_group="g2",
                        batch_state={"status": "running"}))
        session.commit()
        seed_returns("w1", [("web-01", True)])
        seed_returns("w2", [("web-02", True), ("db-01", True)])
        out = run_wave_batch("g2", [["web-01"], ["web-02", "db-01"]],
                             "test.ping", [], 1, "admin", wave_timeout=0)
    assert out["status"] == "complete" and out["failures"] == 0
    assert len(stub.published) == 2
    with app.app_context():
        actions = [e.action for e in get_session().query(AuditEvent)
                   .filter(AuditEvent.action.like("batch-%")).all()]
...
def test_cancel_stops_before_first_wave(monkeypatch, app):
    ...
    with app.app_context():
        get_session().add(Job(jid="batch-g3", fun="test.ping", tgt="*",
                              tgt_type="glob", user="admin", batch_group="g3",
                              batch_state={"status": "running"}))
        get_session().commit()
        out = run_wave_batch("g3", [["web-01"]], "test.ping", [], 1,
                             "admin", wave_timeout=0)
...
def test_batched_run_view_inline(monkeypatch, app, admin):
    ...
    rv = admin.post("/jobs/run", data={
        "tgt": "web-*", "tgt_type": "glob", "fun": "test.ping",
        "mode": "async", "via": "local", "batch_mode": "count",
        "batch_size": "1", "stop_after": "1"})
...
def test_batched_run_rejects_ungroupable_target(admin):
    rv = admin.post("/jobs/run", data={
        "tgt": "role:web", "tgt_type": "compound", "fun": "test.ping",
        "mode": "async", "via": "local", "batch_mode": "count",
        "batch_size": "1", "stop_after": "1"}, follow_redirects=True)
...
def test_cancel_route_sets_flag(monkeypatch, admin):
    ...
    seen = {}
    monkeypatch.setattr(tasks_mod, "request_batch_cancel",
