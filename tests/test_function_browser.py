        ...
        if body.get("client") == "local":
            ...
            if fun == "sys.list_functions":
                ...
                return httpx.Response(200, json={"return": [{
                    "web-01": ["test.ping", "network.interfaces"]}]})
            if fun == "sys.doc":
                return httpx.Response(200, json={"return": [{
                    "web-01": {"network.interfaces":
                               "Return interfaces and respective addresses."}}]})
        return httpx.Response(200, json={"return": [{}]})
...
def test_fun_doc_fragment():
    c = make_client()
    rv = c.get("/jobs/fun-doc", query_string={"fun": "network.interfaces",
                                              "minion": "web-01"})
...
def test_fun_doc_rejects_bad_name():
    c = make_client()
    rv = c.get("/jobs/fun-doc", query_string={"fun": "x;rm -rf",
                                              "minion": "web-01"})
...
def test_viewer_can_read_fun_doc():
    ...
    rv = viewer.get("/jobs/fun-doc", query_string={"fun": "test.ping",
                                                   "minion": "web-01"})
    assert rv.status_code == 200
